try {
    ...
    ! function () {
        ...
        function organizeNavbarMenu() {
            const twoColumnMenu = document.getElementById("two-column-menu");
            const navbarMenu = document.querySelector(".navbar-menu");
            const scrollbar = document.getElementById("scrollbar");
            const navbarNav = document.getElementById("navbar-nav");

            // 🧹 Limpeza e redefinições visuais
            if (twoColumnMenu) twoColumnMenu.innerHTML = "";
            if (navbarMenu) navbarMenu.innerHTML = d; // variável "d" vem do seu contexto original
            if (scrollbar) {
            ...
            const breakpoint = M; // ponto onde o menu será dividido
            let mainItem = null;
            let overflowItemsHTML = "";

            // 🚀 Reorganiza os itens do menu
            navItems.forEach((item, index) => {
                const isAfterBreakpoint = index + 1 > breakpoint;
                if (index + 1 === breakpoint) {
                    mainItem = item;
                } else if (isAfterBreakpoint) {
                    overflowItemsHTML += item.outerHTML;
                    item.remove();
                }
            });
            ...
            function organizeNavbarMenu() {
                ...
                // 🧭 Cria o item “Mais” (More)
                if (mainItem && overflowItemsHTML) {
                    const moreMenuHTML = `
                        <li class="nav-item dropdown fade-in">
                            <a class="nav-link" href="#sidebarMore" data-bs-toggle="collapse" role="button"
                                ...
                                <span data-key="t-more">Mais</span>
                            </a>
                    ...
                    `;
                    mainItem.insertAdjacentHTML("afterend", moreMenuHTML);
                }

                // 💅 Animação de entrada para o novo menu
